...
/****************************************
    Ring Buffer
****************************************/
int ring_len(ring_buffer *buffer) {
  int first_len = buffer->head - buffer->tail;
  if (first_len >= 0) {
    return first_len;
  } else {
    return -first_len - 1;
...
bool ring_empty(ring_buffer *buffer) {
  return buffer->head == buffer->tail;
}

void ring_push(ring_buffer *buffer, char ch) {
  char i = (char) (buffer->head + 1);
...
bool ring_peek(ring_buffer *buffer, char *ch) {
...
bool ring_pop(ring_buffer *buffer, char *ch) {
  if (ring_empty(buffer)) {
    return FALSE;
  }

  *ch = buffer->buffer[buffer->tail];
  char new_tail = buffer->tail + 1;
...
void parse_direction(char *input, int length) {
  char dir = input[0];
  long time = 0;
  if (length > 1) {
    time = strtol(&input[2], NULL, 10);
  }
...
void poll_sensors() {
  ...
  int left = left_sensor.last_val > SENS_LEFT_CUTOFF ? TRUE : FALSE;
  int front = front_sensor.last_val > SENS_FRONT_CUTOFF ? TRUE : FALSE;
  int right = right_sensor.last_val > SENS_RIGHT_CUTOFF ? TRUE : FALSE;
...
bool uart_gets(char *str, int *length) {
  if (!incoming) return FALSE;

  int len = ring_len(&rx_buffer)+1;
  char buf[len];
  unsigned int i = 0;
  for (i; i < len; i++) {
...
void main_100ms_routine() {
  ...
  char ch[UART_BUFFER_SIZE];
  int* len;
  if (uart_gets(ch, len)) {
    parse_direction(ch, *len);
  }
}
